...
check_trait<-function(trait_id){
    ...
    cor_test<-function(tissue_id){
        ...
        rst = cor.test(nlog10p[tissue==tissue_id], rcp[tissue==tissue_id],method="spearman")
        ...
        sig_coloc=which(rcp[tissue==tissue_id]>=0.50)

        sig_both = intersect(sig_twas,sig_coloc)


        pdf(file=paste0("comparison/figures_TWAS_vs_rcp/",trait_id,".",tissue_id,"TWAS_vs_rcp.pdf"), width= 6, height=6,bg="white")
        plot(nlog10p[tissue==tissue_id] ~ rcp[tissue==tissue_id], xlab = "RCP", ylab = "-log10 TWAS p-value", main = paste(trait_id, " ", tissue_id))
        dev.off()
...
write(file="comparison/TWAS_vs_RCP.summary", t(out), ncol=7)
